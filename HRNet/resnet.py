...
class Resnet(nn.Module):
    ...
    def forward(self, x, return_feature_maps=True):
        ...
        x2_up = F.interpolate(
            x2, size=(x0_h, x0_w), mode='bilinear', align_corners=False)
        x3_up = F.interpolate(
            x3, size=(x0_h, x0_w), mode='bilinear', align_corners=False)
        x4_up = F.interpolate(
            x4, size=(x0_h, x0_w), mode='bilinear', align_corners=False)

        conv_out = [x1, x2_up, x3_up, x4_up]
